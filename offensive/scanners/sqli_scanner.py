...
class SQLiScanner(BaseScanner):
    ...
    async def scan(self, context: ScanContext) -> List[Finding]:
        ...
        boolean_findings = await self._scan_boolean_based(context, all_params, post_params, method)
        findings.extend(boolean_findings)
        
        if not findings:
            time_findings = await self._scan_time_based(context, all_params, post_params, method)
            findings.extend(time_findings)
        
        error_findings = await self._scan_error_based(context, all_params, post_params, method)
        findings.extend(error_findings)
        
        union_findings = await self._scan_union_based(context, all_params, post_params, method)
    ...
    async def _scan_boolean_based(
        ...
        for param_name in get_params:
            ...
            length_change_ratio = length_diff / max(true_length, false_length) if max(true_length, false_length) > 0 else 0
    ...
    async def _scan_time_based(
        ...
        for param_name in get_params:
            ...
            baseline_time = await self._measure_response_time(
                context, param_name, get_params[param_name], get_params[param_name], post_params, method
            )
    ...
    async def _send_payload(
        ...
        try:
            if method == "GET":
                return await self.send_request(test_url, method="GET", headers=headers)
            else:
                modified_post = post_params.copy()
                modified_post[param_name] = payload
                return await self.send_request(url, method="POST", data=modified_post, headers=headers)
